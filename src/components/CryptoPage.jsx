...
//     const url = `${endpoint}?vs_currency=${currency}&from=${oneYearAgo}&to=${currentDate}`;

//     fetch(url)
//       .then((response) => response.json())
//       .then((data) => {
//         const prices = data.prices;
//         const highestPrice = Math.max(...prices.map(([timestamp, price]) => price));
//         const lowestPrice = Math.min(...prices.map(([timestamp, price]) => price));
...
//       <Dropdown2
//         label="Currency"
//         arr={supportedCurrencies}
//         fun1={handleCurrency1Change}
//       ></Dropdown2>
//       One {crypto1} is equal to {answer} {currency1}.
//       <br />
//       {highestPrice && <div>Highest BTC price in the last year: {highestPrice}</div>}
//       {lowestPrice && <div>Lowest BTC price in the last year: {lowestPrice}</div>}
//     </ThemeProvider>
//   );
// }
